...
export default {
  ...
  activate (state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'misc-text-tools:parenthesisWrap': () => this.wrapText('(', ')')
    }))
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'misc-text-tools:curlybraceWrap': () => this.wrapText('{', '}')
    }))
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'misc-text-tools:blockSingleQuoteWrap': () => this.wrapText('\'', '\'')
    }))
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'misc-text-tools:blockDoubleQuoteWrap': () => this.wrapText('"', '"')
    }))
  },
  ...
  sortSelection (direction) {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      if (selection.includes(',')) {
        ...
        let chArray = this.sortArray(selection.split(' '), direction)
        selection = chArray.join(' ')
      }
      editor.insertText(selection)
    }
